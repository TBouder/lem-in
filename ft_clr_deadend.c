...
/*   Updated: 2016/05/05 16:06:10 by tbouder          ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
static void	ft_rooms_remove_if(t_rooms **begin_rooms, t_rooms **origin, int *i)
{
	...
	if (*begin_rooms)
	{
		...
		{
			if (ft_pipeslen((*begin_rooms)->pipes_next) == 1)
				ft_useless_pipe_remove_if(&ft_find_room_s(*origin,
					(*begin_rooms)->pipes_next->id)->pipes_next,
					(*begin_rooms)->name);
			to_free = *begin_rooms;
